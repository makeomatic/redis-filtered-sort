...
public:
  ~SortCommand() { delete filters; }

  SortCommand(SortArgs args, RedisModuleCtx *redis, TaskRegistry &registry)
      : args(args), redis(redis), taskRegistry(registry) {
  ...
  string getTaskDescriptor() {
    return pssKey == fflKey
               ? pssKey
               : boost::str(boost::format("%s::%s") % pssKey % fflKey);
  };
  ...
  void waitForPoolEvents(RedisModuleBlockedClient *bc) {
    ...
    auto taskToWait =
        (!sortOnly() && filterTaskExists) ? taskDescriptor : pssKey;

    taskRegistry.registerWaiter(taskToWait,
                                [this, boundProcessCommand, taskToWait, bc]() {
        ...
        threadPool->Enqueue([bc, cmd, thSafeCtx]() {
          try {
            ...
            std::string errMessage =
                boost::str(boost::format("Command error: %s") % e.what());
  ...
  void sortAndFilterData() {
    ...
    filteredData = args.metaKey.empty() ? dataToSort.filter(idFilter)
                                        : dataToSort.filterMeta(filters);
  ...
  bool sortOnly() { return pssKey == fflKey; }
