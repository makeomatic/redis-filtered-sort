...
int FSort_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  std::thread t([bc, argv, argc]() {
    ...
    auto cmd = ms::SortCommand(cmdArgs, redis);
    cmd.execute();
    ms::redis::GlobalUtil::UnblockClient(bc, NULL);
  });
...
extern "C" int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (ms::redis::GlobalUtil::CreateCommand(ctx, "fsort", FSort_RedisCommand, "write deny-script", 1, 2, 1) ==
      REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }

  if (ms::redis::GlobalUtil::CreateCommand(ctx, "fsortBust", FSortBust_RedisCommand, "write deny-script", 1, 1, 1) ==
      REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }

  if (
    ms::redis::GlobalUtil::CreateCommand(ctx, "fsortaggregate", FSortAggregate_RedisCommand, "write deny-script", 1, 2,
                                         1) == REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }
