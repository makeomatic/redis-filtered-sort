...
void SortCommand::init()
{
    ...
    try
    {
        ...
        this->filters.field = "ROOT";
        this->filters.type = FILTERTYPE_OBJECT;
        this->filters.filterFunc = FILTER_UNK;

        this->filters.loadFromJson(this->jsonFilters, 0);
...
int SortCommand::execute(RedisCommander redis)
{
    int pssKType = redis.getKeyType(this->pssKey);
    int fflKType = redis.getKeyType(this->fflKey);
    auto args = this->args;

    if (fflKType == REDISMODULE_KEYTYPE_LIST)
    {
        log("respond FFL key " + this->fflKey + "\n");

        redis.expireKey(this->tempKeysSet, args.expire);
    ...
    if (pssKType == REDISMODULE_KEYTYPE_LIST)
    {
        ...
        {
            log("No filter request respond presorted set");
            redis.expireKey(this->tempKeysSet, args.expire);
            redis.registerCaches(this->pssKey, this->tempKeysSet, args.curTime, args.expire);
    ...
    else
    {
        ...
        if (sortData.size() > 0)
        {
            ...
            if (redis.getKeyType(this->pssKey) == REDISMODULE_KEYTYPE_EMPTY)
            {
    ...
    if (filteredData.size() > 0)
    {
        ...
        if (redis.getKeyType(this->fflKey) == REDISMODULE_KEYTYPE_EMPTY)
        {
