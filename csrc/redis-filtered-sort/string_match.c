...
MatchRange_t *new_match_range(int start, int end) {
  MatchRange_t *mr = malloc(sizeof(MatchRange_t));
  mr->start = start;
  mr->end = end;
  return mr;
}
...
MatchString_t *new_match_string(const char *str, int len) {
  MatchString_t *ms = malloc(sizeof(MatchString_t));
  ms->value = strdup(str);
  ms->size = len;
  return ms;
}
...
Match_t *new_match(MatchType_t t, void * match) {
  Match_t *mt = malloc(sizeof(Match_t));
  mt->type = t;
  mt->value = match;
  return mt;
}
...
int add_match(MatchResult_t *mr, Match_t *m ) {
  mr->size ++;
  mr->data = realloc(mr->data, sizeof(Match_t*) * mr->size);
  mr->data[mr->size-1] = m;
  return 0;
}

MatchResult_t *new_match_result() {
  MatchResult_t *mr = malloc(sizeof(MatchResult_t));
  mr->size= 0;
  mr->data = malloc(sizeof(Match_t*));
  return mr;
}
...
static int seterror(MatchState *ms, const char * fmt, ...) {
  ...
  ms->error_count ++;
  if (ms->error == NULL) {
    ms->error = malloc(sizeof(char*));
  } else {
    ms->error = realloc(ms->error, sizeof(char*) * ms->error_count);
  }
  
  va_list argp;
  va_start(argp, fmt);

  ms->error[ms->error_count -1 ] = malloc(sizeof(char) * 512);
...
static int reprepstate (MatchState *ms) {
  ...
  if (ms->matchdepth != MAXCCALLS) return -1;
  return 0;
}
...
static int singlematch (MatchState *ms, const char *s, const char *p, const char *ep) {
  if (s >= ms->src_end)
    ...
    switch (*p) {
      ...
      case ESC_CHAR: return match_class(c, uchar(*(p+1)));
...
static const char *matchbalance (MatchState *ms, const char *s, const char *p) {
  if (p >= ms->p_end - 1) seterror(ms, "malformed pattern (missing arguments to '%%b')");
  if (*s != *p) return NULL;
  else {
    int b = *p;
    int e = *(p+1);
...
static const char *start_capture (MatchState *ms, const char *s, const char *p, int what) {
  const char *res;
  int level = ms->level;
  if (level >= MAXCAPTURES) seterror(ms, "too many captures");
...
static const char *match (MatchState *ms, const char *s, const char *p) {
  ...
  if (p != ms->p_end) {  
    switch (*p) {
      ...
      case ESC_CHAR: {  // escaped sequences not in the format class[*+?-]?
        switch (*(p + 1)) {
          ...
          case 'f': {  // frontier?
            ...
            p += 2;
            if (*p != '[') seterror(ms, "missing '[' after '%%f' in pattern");
...
int print_result(MatchResult_t * mr) {
  ...
  if (mr->error_count > 0) {
    for (size_t i = 0; i < mr->error_count; i++)
    {
